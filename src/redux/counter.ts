...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addToCart: (state, action: PayloadAction<{ id: number; name: string; price: number, image: string }>) => {
      ...
      if (!userData) {
        Error("Please log in to add items to the cart.");
        return;
      }
      ...
      if (itemIndex >= 0) {
        ...
        info(`${action.payload.name} quantity increased`);
      } else {
    ...
    addToFavorite: (state, action: PayloadAction<{ id: number; name: string; price: number, image: string }>) => {
      ...
      const userData = state.userData; // الحصول على بيانات المستخدم من الـ Redux state
      if (!userData) {
        Error("Please log in to add items to the my favourite.");
        return;
      }
      ...
      if (itemIndex >= 0) {
        info(`this item is already added to my favourite`);
      } else {
        const tempItem = { ...action.payload};
        state.favoriteItems.push(tempItem);
        success(`Item added to my favourite`);
      }
  ...
  DeleteFromFavorite: (state, action: PayloadAction<{ id: number }>) => {
      const userData = state.userData; // الحصول على بيانات المستخدم من الـ Redux state
      if (!userData) {
        Error("Please log in to remove items from the my favorite.");
        return;
      }
      ...
      Error(`Item removed from my favorite`)
      
    },
    removeFromCart: (state, action: PayloadAction<{ id: number }>) => {
      const userData = state.userData; // الحصول على بيانات المستخدم من الـ Redux state
      if (!userData) {
        Error("Please log in to remove items from the cart.");
        return;
      }
